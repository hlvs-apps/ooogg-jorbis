...
class Residue0{
	public static Residue0[] residue_P = {new Residue0(), new Residue1(), new Residue2()};
	...
	Object unpack(Info vi, Buffer opb){
		...
		for(int j = 0; j < acc; j++) info.booklist[j] = opb.read(8);
		if(info.groupbook >= vi.books) return null;
		for(int j = 0; j < acc; j++){
			if(info.booklist[j] >= vi.books) return null;
		}
		return info;
	}
	...
	Object look(DspState vd, InfoMode vm, Object vr){
		...
		dim = look.phrasebook.getCodebookDimensions();
		...
		for(int j=0; j < look.parts; j++){
			...
			if(stages != 0){
				...
				for(int k=0; k < stages; k++){
					if((i & (1 << k)) != 0) look.partbooks[j][k] = info.booklist[acc++];
				}
			}
		}
		...
		for(int j=0; j < look.partvals; j++){
			int val = j, mult = look.partvals / look.parts;
	...
	int inverse(Block vb, Object vl, float[][] in, int[] nonzero, int ch){
		int used = 0;
		for(int i = 0; i < ch; i++) if(nonzero[i] != 0) in[used++] = in[i];
		return used == 0 ? 0 : _01inverse(vb, vl, in, used, 0);
	}
	...
	synchronized static int _01inverse(Block vb, Object vl, float[][] in, int ch, int decodepart) {
		...
		if(_01inverse_partword.length < ch) _01inverse_partword = new int[ch][][];
		for(j = 0; j < ch; j++){
		...
		for(s = 0; s < look.stages; s++){
			// each loop decodes on partition codeword containing partitions_pre_word partitions
			for(i = 0, l = 0; i < partvals; l++){
				if(s == 0){
					...
					for(j = 0; j < ch; j++){
						...
						if(_01inverse_partword[j][l] == null) return 0;
					}
				}
				...
				for(k = 0; k < partitions_per_word && i < partvals; k++, i++){
					for(j = 0; j < ch; j++){
						...
						if((info.secondstages[index] & (1 << s)) != 0){
							...
							if(stagebook != null){
								if(decodepart == 0){
									if(stagebook.decodevs_add(in[j], offset, vb.opb, samples_per_partition) == -1) return 0;
								}else if(decodepart == 1){
									if(stagebook.decodev_add(in[j], offset, vb.opb, samples_per_partition) == -1) return 0;
								}
							}
						}
					}
