...
class Block{
	...
	public int synthesis(OggPacket op){
		...
		if(opb.read(1) != 0) return -1; // Oops. This is not an audio data packet
		// read our mode and pre/post windowsize
		int _mode = opb.read(vd.modebits);
		if(_mode == -1) return -1;
		mode = _mode;
		W = vd.vi.mode_param[mode].blockflag;
		if(W != 0){
			lW = opb.read(1);
			nW = opb.read(1);
			if(nW == -1) return -1;
		...
		pcmend = vd.vi.blocksizes[W];
		if(pcm.length < vd.vi.channels) pcm = new float[vd.vi.channels][];
		for(int i = 0; i < vd.vi.channels; i++){
		...
		return Mapping0.mapping_P[vd.vi.map_type[vd.vi.mode_param[mode].mapping]].inverse(this, vd.mode[mode]);
	}
}