...
class DspState{
	...
	static float[] window(int type, int window, int left, int right){
		...
		case 0:
			...
			for(int i = 0; i < left; i++){
				float x = (float) ((i + .5) / left * Lookup.M_PI / 2.);
				x = (float) Math.sin(x);
				x *= x;
				x *= Lookup.M_PI / 2.;
				x = (float) Math.sin(x);
			...
			for(int i = leftbegin + left; i < rightbegin; i++) ret[i] = 1.f;
			for(int i = 0; i < right; i++){
				float x = (float) ((right - i - .5) / right * Lookup.M_PI / 2.);
				x = (float) Math.sin(x);
				x *= x;
				x *= Lookup.M_PI / 2.;
				x = (float) Math.sin(x);
	...
	int init(Info vi, boolean encp){
		...
		pcm_storage = 8192; // we'll assume later that we have a minimum of twice the blocksize of accumulated samples in analysis
		pcm = new float[vi.channels][];
		for(int i = 0; i < vi.channels; i++) pcm[i] = new float[pcm_storage];
		...
		for(int i = 0; i < vi.modes; i++){
			...
			mode[i] = Mapping0.mapping_P[maptype].look(this, vi.mode_param[i], vi.map_param[mapnum]);
		}
		return 0;
	}
	...
	// Unike in analysis, the window is only partially applied for each block. The time domain envelope
	// is not yet handled at the point of calling (as it relies on the previous block).
	public int synthesis_blockin(Block vb){
		// Shift out any PCM/multipliers that we returned previously centerW is currently the center of the last block added
		if(centerW > vi.blocksizes[1] / 2 && pcm_returned > 8192){
			// don't shift too much; we need to have a minimum PCM buffer of 1/2 long block
			...
			if(shiftPCM != 0){
				for(int i = 0; i < vi.channels; i++) System.arraycopy(pcm[i], shiftPCM, pcm[i], 0, pcm_current);
			}
		}
		lW = W;
		W = vb.W;
		...
		sequence = vb.sequence;
		{
			...
			for(int j = 0; j < vi.channels; j++){
				...
				int i = 0;
				for(i = beginSl; i < endSl; i++) pcm[j][_pcm + i] += vb.pcm[j][i];
				// the remaining section
				for(; i < sizeW; i++) pcm[j][_pcm + i] = vb.pcm[j][i];
			}
			// Track the frame number... This is for convenience, but also making sure our last packet doesn't end with added padding.
			// If the last packet is partial, the number of samples we'll have to return will be past the vb->granulepos.
			// This is not foolproof! It will be confused if we begin decoding at the last page after a seek or hole. In that case,
			// we don't have a starting point to judge where the last frame is. For this reason, vorbisfile will always try to make
			// sure it reads the last two marked pages in proper sequence.
			if(granulepos == -1){
	...
	public int synthesis_pcmout(float[][][] _pcm, int[] index) {
		if(pcm_returned < centerW){
			if(_pcm != null) {
				for(int i = 0; i < vi.channels; i++) index[i] = pcm_returned;
				_pcm[0] = pcm;
			}
